...
void newEntry(tableEntry* entryPtr)
	...
	entryPtr->value = temp ;
	entryPtr->hashAddress = abs(temp%HASHSIZE) ; // use abs() in case of -ve value
	printf( "Value: %5ld \t HashValue: %5d \n", entryPtr->value, entryPtr->hashAddress ) ;
	} // newEntry()

/***************************************************/
int searchRow(tableEntry* entryPtr)
	/* searches for the value in row 'hashAddress' that's pointed to by entryPtr
...
void addNodeToStart(tableEntry* entryPtr)
	/* insert the new node at the start of the row */
	{
